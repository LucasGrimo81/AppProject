...
function Home() {
  ...
  {/* GESTION DE COLORES POR PRIORIDAD Y RIESGO */}
  const getPriorityColor = (priority) => {
    ...
    {/* AJUSTA COLUMNAS DE TABLA DE TEST CASES */}
    ...
    const exportToExcel = (response) => {
      ...
      const summaryData = [
        ["Field", "Value"],
        ["User Story", response.title],
        ["Acceptance Criteria", response.acceptance_criteria],
    ...
    //FORMATEAR AC SI ES NECESARIO PERO AHORA ESTAMOS RENDERIZANDO EN UI
 /*   const formatAC = (ac) => {
      if (!ac) return "";

      // 🟢 si ya es string → dejarlo
      if (typeof ac === "string") return ac;

      // 🟢 si es array de escenarios
      if (Array.isArray(ac)) {
        return ac.map(s => `
    Given ${s.given}
    When ${s.when}
    ...
    const formattedAC = formatAC(response?.acceptance_criteria); 
    */
                    
    {/* FETCH DE LAS SUGGESTIONS */}
    const loadSuggestions = async (data) => {
      ...
      const res = await fetch("http://localhost:3000/suggestions", {
        ...
        body: JSON.stringify({
          user_story: data.user_story || data.title,
    ...
    //AGREGAR TCs SUGERIDOS 
    const addSuggestionAsTC = async (suggestion) => {
       console.log("🔥 CLICK ADD TC", suggestion);
      const newTC = {
      ...
      // 🔴 recalcular métricas
      await fetch("http://localhost:3000/history/recalculate ", {
        method: "POST"
      });
    ...
    const acList = Array.isArray(response?.acceptance_criteria)
          ...
          <div className="space-y-4">
            ...
            {response.title && (
              <div className="bg-white p-4 rounded-lg shadow">
                <h2 className="font-semibold">User Story: {response.title}</h2>
            ...
            {suggestions.length > 0 && (
              <div className="mt-6">
                <h3 className="font-bold mb-2">💡 Suggested Test Scenarios</h3>

                <div className="space-y-2">
                  {suggestions.map((s, i) => (
                    <div
                      key={i}
                      className="p-3 bg-yellow-50 border rounded flex justify-between items-center"
                    >
                      <div>
